...
function addProduct() {
    ...
    const newRow = `
        <tr>
            ...
            <td><input type="number" placeholder="Unit" oninput="updateProduct(this)"></td>
...
function updateAmount(input) {
    ...
    const priceInput = row.children[2].children[0];
    const quantityInput = row.children[3].children[0];
...
function updateProduct(input, field) {
    ...
    const Unit = row.children[1].children[0];
    const priceInput = row.children[2].children[0];
    const quantityInput = row.children[3].children[0];
    ...
    if (products[productIndex]) {
        products[productIndex] = { productName: productName, unit:Unit, unitPrice: price, quantity: quantity };
    } else {
        products.push({ productName: productName, unit:Unit, unitPrice: price, quantity: quantity });
    }
}
...
function downloadInvoice() {
    ...
    productRows.forEach(row => {
        ...
        const quantity = parseInt(row.children[3].children[0].value) || 0;
    ...
    .then(blob => {
        ...
        a.href = url;
        a.download = customerName+'.pdf';  // Set the default file name
